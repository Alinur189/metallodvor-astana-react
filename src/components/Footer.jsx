import { Link } from 'react-router-dom';
import { categories } from '../data/categories.js';
import logo from '../assets/logo.jpg';

export default function Footer() {
  return (
    <footer className="footer">
      <div className="container footer__grid">
        <div>
          <Link className="logo logo--footer" to="/">
            <img src={logo} alt="МеталлоДвор Астана" className="logo__img" />
          </Link>
          <p>
            ТОО «МеталлоДвор» — более 20 лет на рынке. Поставляем металлическую сетку, арматуру
            и металлопрокат для строительных компаний, подрядчиков, складов и частных объектов в Астане.
          </p>
        </div>

        <div>
          <h3>Каталог</h3>
          <ul>
            {categories.slice(0, 5).map((category) => (
              <li key={category.slug}>
                <Link to={`/catalog/${category.slug}`}>{category.title}</Link>
              </li>
            ))}
          </ul>
        </div>

        <div>
          <h3>Навигация</h3>
          <ul>
            <li><Link to="/about">О компании</Link></li>
            <li><Link to="/delivery">Доставка</Link></li>
            <li><Link to="/price-list">Прайс-лист</Link></li>
            <li><Link to="/contacts">Контакты</Link></li>
          </ul>

        </div>




        <div>
          <h3>Контакты</h3>
          <ul>
            <li><a href="tel:+77021665051">+7 702 166 5051</a> — Қуаныш</li>
            <li><a href="tel:+77015877127">+7 701 587 7127</a> — Нуржан (сетка и завод)</li>
            <li><a href="mailto:metallodvorastana@gmail.com">metallodvorastana@gmail.com</a></li>
            <li>Астана, Рынок "Эталон ряд 15, место 13"</li>
          </ul>
        </div>
      </div>

      <div className="container footer__bottom">
        <span>© 2026 ТОО «МеталлоДвор» • на рынке с 2006 года</span>
        <span>Астана, Рынок «Эталон», ряд 15, место 13</span>
        <span className="footer__credits">
          Фото товаров:{' '}
          <a href="https://commons.wikimedia.org/" target="_blank" rel="noopener noreferrer">Wikimedia Commons</a>
          {' '}(
          <a href="https://creativecommons.org/licenses/by-sa/4.0/" target="_blank" rel="noopener noreferrer">CC BY-SA 4.0</a>
          )
        </span>
      </div>
    </footer>
  );
}
